...
def is_it_OK(subject_code, current_price):
    ...
    contract_cnt = 1
    '''
    if str(calc.data[subject_code]['체결시간'][-1])[:10] == '2017051811':
        print(str(siga) + ' / ' + str(goga) + ' / ' + str(juga) + ' / ' + str(jonga) + ' / ' + str(current_price))
        print(calc.data[subject_code]['금일'])
    '''
    ...
    if jonga <= calc.data[subject_code]['금일']['시가']:# and siga < calc.data[subject_code]['금일']['시가']: # 이전캔들이양봉임, 매도 가능
        if current_price >= calc.data[subject_code]['금일']['시가']:
            mesu_medo_type = '신규매도'
            pass # 매도

    if jonga >= calc.data[subject_code]['금일']['중심선'] and current_price >= calc.data[subject_code]['금일']['시가']: #이전캔들이 음봉, 매수 가능
        if current_price <= calc.data[subject_code]['금일']['중심선']:
            mesu_medo_type = '신규매수'
            pass # 매수

    if jonga <= calc.data[subject_code]['금일']['중심선'] and current_price <= calc.data[subject_code]['금일']['시가']: # 이전캔들이양봉임, 매도 가능
        if current_price >= calc.data[subject_code]['금일']['중심선']:
            mesu_medo_type = '신규매도'
            pass # 매도     
    
    if mesu_medo_type == '': return false

    order_contents = {'신규주문':True, '매도수구분':mesu_medo_type, '익절틱':20, '손절틱':50, '수량':contract_cnt}
...
def is_it_sell(subject_code, current_price):
    ...
    pre_siga = calc.data[subject_code]['시가'][-2]
    pre_goga = calc.data[subject_code]['고가'][-2]
    pre_juga = calc.data[subject_code]['저가'][-2]
    pre_jonga = calc.data[subject_code]['현재가'][-2]

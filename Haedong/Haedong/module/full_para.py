...
def is_it_OK(subject_code, current_price):
    ...
    if subject.info[subject_code]['상태'] == '매수가능' :
        ...
        if calc.data[subject_code]['매매가능가'] >= current_price or '1'=='1':
            if current_price >= min(calc.data[subject_code]['일목균형표']['선행스팬1'][calc.data[subject_code]['idx']],calc.data[subject_code]['일목균형표']['선행스팬2'][calc.data[subject_code]['idx']]) or '1'=='1':#필요없음
                if calc.data[subject_code]['이동평균선'][30][-1] > calc.data[subject_code]['이동평균선'][60][-1] or '1'=='1':
        ...
        if calc.data[subject_code]['매매가능가'] <= current_price or '1'=='1':
            if current_price <= max(calc.data[subject_code]['일목균형표']['선행스팬1'][calc.data[subject_code]['idx']],calc.data[subject_code]['일목균형표']['선행스팬2'][calc.data[subject_code]['idx']]) or '1'=='1': #필요없음
                if calc.data[subject_code]['이동평균선'][30][-1] < calc.data[subject_code]['이동평균선'][60][-1] or '1'=='1':
                    #if calc.data[subject_code]['이동평균선'][100][-30] - calc.data[subject_code]['이동평균선'][100][-1] > 0:
                    if calc.data[subject_code]['이동평균선'][100][-30] - calc.data[subject_code]['이동평균선'][100][-1] >= 0 or '1'=='1':
                        if calc.data[subject_code]['이동평균선'][200][-1] > calc.data[subject_code]['이동평균선'][60][-1] or '1' == '1':
        ...
        if calc.data[subject_code]['매매가능가'] + minimum_reverse_dribble_tic*subject.info[subject_code]['단위'] <= current_price or '1'=='1':
            ...
            if calc.data[subject_code]['이동평균선'][100][-30] < calc.data[subject_code]['이동평균선'][100][-15] < calc.data[subject_code]['이동평균선'][100][-7] < calc.data[subject_code]['이동평균선'][100][-1] or '1'=='1':
                if calc.data[subject_code]['이동평균선'][150][-1] < calc.data[subject_code]['이동평균선'][60][-1]:
                    ...
                    if current_price - max(calc.data[subject_code]['일목균형표']['선행스팬1'][calc.data[subject_code]['idx']],calc.data[subject_code]['일목균형표']['선행스팬2'][calc.data[subject_code]['idx']]) > subject.info[subject_code]['단위']/2 or '1'=='1':
                        if calc.data[subject_code]['이동평균선'][100][-1] - calc.data[subject_code]['이동평균선'][300][-1] < subject.info[subject_code]['단위']*16 or '1' == '1': 
                            ...
                            log.info("100일선과 300일선이 너무 멀어 매수 포기")
                            subject.info[subject_code]['상태'] = '매매완료'        
                            return {'신규주문':False}  
                    else:
                        return {'신규주문':False}
        ...
        if calc.data[subject_code]['매매가능가'] - minimum_reverse_dribble_tic*subject.info[subject_code]['단위'] >= current_price or '1'=='1':
            ...
            if calc.data[subject_code]['이동평균선'][100][-30] > calc.data[subject_code]['이동평균선'][100][-15] > calc.data[subject_code]['이동평균선'][100][-7] > calc.data[subject_code]['이동평균선'][100][-1] or '1'=='1':
                if calc.data[subject_code]['이동평균선'][150][-1] > calc.data[subject_code]['이동평균선'][60][-1]:
                    ...
                    if min(calc.data[subject_code]['일목균형표']['선행스팬1'][calc.data[subject_code]['idx']],calc.data[subject_code]['일목균형표']['선행스팬2'][calc.data[subject_code]['idx']]) - current_price > subject.info[subject_code]['단위']/2 or '1'=='1':
                        if calc.data[subject_code]['이동평균선'][300][-1] - calc.data[subject_code]['이동평균선'][100][-1] < subject.info[subject_code]['단위']*16 or '1' == '1' :
            ...
            if current_price >= contract.list[subject_code]['익절가']: 
                if contract.list[subject_code]['계약타입'][contract.DRIBBLE] > 0:
                    ...
                    if get_time(30,subject_code) >= int(subject.info[subject_code]['마감시간']) and get_time(0,subject_code) < int(subject.info[subject_code]['마감시간']):
                        ...
                        contract.list[subject_code]['익절가'] = current_price + ( 50*subject.info[subject_code]['단위'] )
                    ...
                    #    contract.list[subject_code]['손절가'] = contract.list[subject_code]['체결가']  
                    
                    #elif current_price >= contract.list[subject_code]['체결가'] + 30*subject.info[subject_code]['단위']:
                    #    log.info("드리블 목표 달성으로 익절가 수정. 15틱 이상 익절로 5틱 캡으로 드리블")
                    #    contract.list[subject_code]['익절가'] = current_price + ( subject.info[subject_code]['단위'] )
                    #    contract.list[subject_code]['손절가'] = current_price - (25*subject.info[subject_code]['단위'])                    
                    #####

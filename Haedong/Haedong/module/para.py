...
def is_it_OK(subject_code, current_price):
    ...
    subject.info[subject_code]['sar매매틱간격'] = 5
    ...
    if subject.info[subject_code]['flow'] == '상향' and subject.info[subject_code]['상태'] != '매도가능': 
        ...
        if current_price > subject.info[subject_code]['sar']:
            if current_price > min(calc.data[subject_code]['일목균형표']['선행스팬1'][calc.data[subject_code]['idx']],calc.data[subject_code]['일목균형표']['선행스팬2'][calc.data[subject_code]['idx']]):
                log.info("")
                log.info('상태 변경, ' + subject.info[subject_code]['상태'] + ' -> 매수가능, 현재가:'+str(current_price) + '시간:'+str(""))
    ...
    if subject.info[subject_code]['상태'] == '매수가능' :
        ...
        if calc.data[subject_code]['매매가능가'] >= current_price:
            if current_price >= min(calc.data[subject_code]['일목균형표']['선행스팬1'][calc.data[subject_code]['idx']],calc.data[subject_code]['일목균형표']['선행스팬2'][calc.data[subject_code]['idx']]) - subject.info[subject_code]['단위']*3:#필요없음
                    ...
                    if calc.data[subject_code]['이동평균선'][100][-30] - calc.data[subject_code]['이동평균선'][100][-1] < 0:
                        ...
                        if calc.data[subject_code]['이동평균선'][200][-1] < calc.data[subject_code]['이동평균선'][60][-1]:
                            ...
                            #log.info(calc.data[subject_code]['이동평균선'][200][-1])
                                ...
                                current_price = calc.data[subject_code]['매매가능가']
                                return {'신규주문':False}
                            else:
                ...
                log.info("현재가가 일목균형표 위에 위치 하지 않아 매도 시도 하지 않습니다. 이번 flow skip 합니다.")
                subject.info[subject_code]['상태'] = '매매완료'                
                return {'신규주문':False}
        ...
        '''
        if str(calc.data[subject_code]['체결시간'][-1])[-6:-2] == '0900':
            log.info('현재가:' + str(current_price))
            log.info('체결시간: ' + str(calc.data[subject_code]['체결시간'][-1]))
            log.info('ilmok_min : ' + str(ilmok_min))
            log.info('매매가능가: ' + str(calc.data[subject_code]['매매가능가']))
            log.info('선행스팬1: ' + str(calc.data[subject_code]['일목균형표']['선행스팬1'][calc.data[subject_code]['idx']]))
            log.info('선행스팬2: ' + str(calc.data[subject_code]['일목균형표']['선행스팬2'][calc.data[subject_code]['idx']]))
        '''                                                            
        #if min(calc.data[subject_code]['매매가능가'], calc.data[subject_code]['이동평균선'][30][-1]) <= current_price:
        if calc.data[subject_code]['매매가능가'] <= current_price:
            if current_price <= max(calc.data[subject_code]['일목균형표']['선행스팬1'][calc.data[subject_code]['idx']],calc.data[subject_code]['일목균형표']['선행스팬2'][calc.data[subject_code]['idx']]) + subject.info[subject_code]['단위']*3: #필요없음
                if calc.data[subject_code]['이동평균선'][30][-1] < calc.data[subject_code]['이동평균선'][60][-1]:
                    #if calc.data[subject_code]['이동평균선'][100][-30] - calc.data[subject_code]['이동평균선'][100][-1] > 0:
                    if calc.data[subject_code]['이동평균선'][100][-30] - calc.data[subject_code]['이동평균선'][100][-1] >= 0:
                        if calc.data[subject_code]['이동평균선'][200][-1] > calc.data[subject_code]['이동평균선'][60][-1]:
        ...
        if calc.data[subject_code]['매매가능가'] + 2*subject.info[subject_code]['단위'] <= current_price:
            ...
            log.info("매매가능가 갱신, 현재가 하향 중, 현재가:%s" % current_price)
            return {'신규주문':False}
        else: return {'신규주문':False}
        ...
        if calc.data[subject_code]['매매가능가'] - 2*subject.info[subject_code]['단위'] >= current_price:
            ...
            calc.data[subject_code]['매매가능가'] = current_price
            log.info("매매가능가 갱신, 현재가 상향 중, 현재가:%s" % current_price)
            return {'신규주문':False}
        else: return {'신규주문':False}    
...
def is_it_sell(subject_code, current_price):
    ...
    if contract.get_contract_count(subject_code) > 0:
        ...
        if contract.list[subject_code]['매도수구분'] == '신규매수':
            ...
            if current_price >= contract.list[subject_code]['익절가']: 
                if contract.list[subject_code]['계약타입'][contract.DRIBBLE] > 0:
                    ...
                    ######
                        ...
                        contract.list[subject_code]['익절가'] = current_price + ( subject.info[subject_code]['단위'] )
                        contract.list[subject_code]['손절가'] = current_price - ( (subject.info[subject_code]['주문내용']['손절틱'] - 2) * subject.info[subject_code]['단위'] ) # 수수료 때문에 1틱 뺌
            ...
            elif current_price < min(calc.data[subject_code]['일목균형표']['선행스팬1'][calc.data[subject_code]['idx']],calc.data[subject_code]['일목균형표']['선행스팬2'][calc.data[subject_code]['idx']]) - 4*subject.info[subject_code]['단위']:
            ...
            elif current_price > max(calc.data[subject_code]['일목균형표']['선행스팬1'][calc.data[subject_code]['idx']],calc.data[subject_code]['일목균형표']['선행스팬2'][calc.data[subject_code]['idx']]) + 4*subject.info[subject_code]['단위']:
                log.info("일목균형 이탈하여 " + str(contract.list[subject_code]['계약타입'][contract.SAFE] + contract.list[subject_code]['계약타입'][contract.DRIBBLE]) + "개 청산 요청.")
                return {'신규주문':True, '매도수구분':'신규매수', '수량':contract.list[subject_code]['계약타입'][contract.SAFE] + contract.list[subject_code]['계약타입'][contract.DRIBBLE]}

...
def is_it_OK(subject_code, current_price):
    ...
    if subject.info[subject_code]['상태'] == '매수중' or subject.info[subject_code]['상태'] == '매도중' or subject.info[subject_code]['상태'] == '청산시도중' or subject.info[subject_code]['상태'] == '매매시도중' or subject.info[subject_code]['상태'] == '매매완료':
    ...
    if calc.subject.info[subject_code]['맞틀리스트'][-3:] == ['틀', '틀', '틀']:
        if subject.info[subject_code]['flow'] == '상향': 
            if current_price >= calc.data[subject_code]['추세선밴드']['상한선'][-1]:
                res.info('상향, 반대매도(' + str(current_price) + ')');
                mesu_medo_type = '신규매도'
                sonjal_tick = 9999
            else: return false
        elif subject.info[subject_code]['flow'] == '하향': 
            if current_price <= calc.data[subject_code]['추세선밴드']['하한선'][-1]:
                res.info('하향, 반대매수(' + str(current_price) + ')');
                mesu_medo_type = '신규매수'
                sonjal_tick = 9999
            else: return false
        else: return false
    ...
    if len(calc.data[subject_code]['SAR반전시간']) > 0 and calc.data[subject_code]['SAR반전시간'][-1] == calc.data[subject_code]['체결시간'][-1]: # 반전 후 SAR로 갱신되었다면
        if subject.info[subject_code]['맞틀리스트'][-3:] == ['틀','틀','틀']: return false 
    else:
        if subject.info[subject_code]['맞틀리스트'][-2:] == ['틀','틀'] and profit < 0: return false
...
def is_it_sell(subject_code, current_price):
    ...
    if contract.get_contract_count(subject_code) > 0:
        ...
        if contract.list[subject_code]['매도수구분'] == '신규매수':
            # 매수일때
            if contract.list[subject_code]['손절가'] == 0:
                if current_price <= calc.data[subject_code]['추세선밴드']['청산하한선'][-1]:
                    res.info("청산하한가(손절)가되어 반대매매한 " + str(contract.list[subject_code]['계약타입'][contract.SAFE] + contract.list[subject_code]['계약타입'][contract.DRIBBLE]) + "개 청산 요청.")
                ...
                elif current_price >= calc.data[subject_code]['추세선밴드']['상한선'][-1]:
                    res.info("목표상한가가 되어 반대매매한 " + str(contract.list[subject_code]['계약타입'][contract.SAFE] + contract.list[subject_code]['계약타입'][contract.DRIBBLE]) + "개 청산 요청.")
                    return {'신규주문':True, '매도수구분':'신규매도', '수량':contract.list[subject_code]['계약타입'][contract.SAFE] + contract.list[subject_code]['계약타입'][contract.DRIBBLE]}
                elif calc.data[subject_code]['플로우'][-1] == '상향' and subject.info[subject_code]['sar'] > current_price:
                    res.info("상향 반전되어 반대매매한 " + str(contract.list[subject_code]['계약타입'][contract.SAFE] + contract.list[subject_code]['계약타입'][contract.DRIBBLE]) + "개 청산 요청.")
                    return {'신규주문':True, '매도수구분':'신규매도', '수량':contract.list[subject_code]['계약타입'][contract.SAFE] + contract.list[subject_code]['계약타입'][contract.DRIBBLE]}
            else:
            ...
            # 매도일때
            if contract.list[subject_code]['손절가'] == 0:
                if current_price >= calc.data[subject_code]['추세선밴드']['청산상한선'][-1]:
                    res.info("청산상한가(손절)가되어 반대매매한 " + str(contract.list[subject_code]['계약타입'][contract.SAFE] + contract.list[subject_code]['계약타입'][contract.DRIBBLE]) + "개 청산 요청.")
                ...
                elif current_price <= calc.data[subject_code]['추세선밴드']['하한선'][-1]:
                    res.info("목표하한가가 되어 반대매매한 " + str(contract.list[subject_code]['계약타입'][contract.SAFE] + contract.list[subject_code]['계약타입'][contract.DRIBBLE]) + "개 청산 요청.")
                    return {'신규주문':True, '매도수구분':'신규매수', '수량':contract.list[subject_code]['계약타입'][contract.SAFE] + contract.list[subject_code]['계약타입'][contract.DRIBBLE]}
                elif calc.data[subject_code]['플로우'][-1] == '하향' and subject.info[subject_code]['sar'] < current_price:
                    res.info("하향 반전되어 반대매매한 " + str(contract.list[subject_code]['계약타입'][contract.SAFE] + contract.list[subject_code]['계약타입'][contract.DRIBBLE]) + "개 청산 요청.")
                    return {'신규주문':True, '매도수구분':'신규매수', '수량':contract.list[subject_code]['계약타입'][contract.SAFE] + contract.list[subject_code]['계약타입'][contract.DRIBBLE]}

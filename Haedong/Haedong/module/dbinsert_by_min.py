...
import gmail, log, calc, santa, screen, db_min
import json
        ...
        if sRQName == "해외선물옵션분차트조회":
            ...
            if sScrNo == dbsubject.info[subject_code]['화면번호']:                    
                ...
                if int(self.recent_date) < int(_data[6]):
                    ...
                    db_min.insert(self.data, self.start_date, subject_code)
                    time.sleep(5)
                    ...
                    if len(subject_codes)>0:
                        subject_code=subject_codes[0]
                        self.request_min_info(subject_code,1, "")
                    else :
                        quit()
...
#                 print('today_time',today_time)
                ...
                if int(_data[2][:12]) < int(self.start_date + dbsubject.info[subject_code]['시작시간']):
                    ...
                    time.sleep(0.2)
                    self.request_min_info(subject_code,1, sPreNext)   
            '''
...
#                     print('today_time',today_time)
        ...
        if sRQName == '상품별현재가조회':
            ...
            self.request_min_info(subject_codes[0],1, "")        
            # 초기 데이터 요청
    ...
    def OnReceiveRealData(self, sSubjectCode, sRealType, sRealData):
        ...
        """
        # 캔들 생기는 시점 확인해서 가격이 안바뀌어도 옥수수에 3틱으로 설정할 경우 가격변동없이 캔들이 생기는 경우가 있으니 request_min_info 시점 확인
